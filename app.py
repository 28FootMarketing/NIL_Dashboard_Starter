...
if is_admin:
    st.sidebar.markdown("🚨 **Admin Mode Enabled**", unsafe_allow_html=True)
else:
    if st.session_state.get("partner_mode", False):
        st.sidebar.markdown("🤝 **Partner Mode Active**", unsafe_allow_html=True)
...
logo_url = partner_config.get("logo_url")
if logo_url:
    st.image(logo_url, width=150)

# ✅ Visual Progress Bar (Steps 1-7)
steps_total = 7
steps_enabled = sum([toggle_states.get(f"step_{i}", False) for i in range(1, 8)])
st.progress(steps_enabled / steps_total)

# ✅ Step 0: NIL Education (Always Shown)
with st.expander("🎓 NIL Education", expanded=True):
    run_nil_course()

# ✅ Step 1: NIL Readiness Quiz
if toggle_states.get("step_1", True):
    with st.expander("📋 Step 1: NIL Readiness Quiz", expanded=True):
...
if toggle_states.get("step_2", True):
    with st.expander("💼 Step 2: NIL Business Tools", expanded=True):
...
if toggle_states.get("step_3", True):
    with st.expander("🧾 Step 3: NIL Deal Builder Wizard", expanded=True):
        run_wizard()

# ✅ Step 4: Pitch Deck Generator
if toggle_states.get("step_4", True):
    with st.expander("📊 Step 4: NIL Pitch Deck Generator", expanded=True):
...
if toggle_states.get("step_5", True):
    with st.expander("📅 Step 5: Weekly Content Plan", expanded=True):
        display_calendar()

# ✅ Step 6: NIL Success Stories
if toggle_states.get("step_6", True):
    with st.expander("📚 Step 6: Real NIL Success Stories", expanded=True):
        show_case_studies()

# ✅ Step 7: Contact Form
if toggle_states.get("step_7", True):
    with st.expander("📥 Step 7: Stay in the NIL Loop", expanded=True):
...
# ✅ Always Show Leaderboard (with optional filter)
with st.sidebar:
    filter_gold = st.checkbox("Filter: Gold Partners Only")

with st.expander("🏆 NIL Leaderboard", expanded=True):
    if filter_gold:
        display_leaderboard(filter_by_tier="Gold")
    else:
        display_leaderboard()
